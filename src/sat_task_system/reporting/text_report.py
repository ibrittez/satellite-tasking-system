"""One run's summary as plain text. No IO, so every caller decides where it goes."""
...
def render_summary(summary: Summary) -> str:
    """The whole report as one string, ready for a terminal or a <pre> block."""
    outcomes = {result.task_name: result.success for result in summary.results}

    assigned = sum(len(assignment.tasks) for assignment in summary.assignments)
    ...
    for assignment in summary.assignments:
        ...
        for task in assignment.tasks:
            mark = f"[{OK_MARK}]" if outcomes[task.name] else f"[{FAIL_MARK}]"
            lines.append(_row(mark, task))

    if summary.skipped:
        lines.append("\nskipped:")
        lines.extend(_row("", task) for task in summary.skipped)
...
def _row(mark: str, task: Task) -> str:
    ...
    resources = ", ".join(str(resource) for resource in sorted(task.resources))
